...
final class FlagStore {
    ...
    private let exactSourceMap: [String: String] = [
        ...
        "com.apple.keylayout.byelorussian": "BY",
        "com.apple.keylayout.belarusian": "BY",
    ...
    func image(sourceID: String, languageCode: String) -> NSImage {
        let country = mappedCountryCode(sourceID: sourceID, languageCode: languageCode)
    ...
    private func mappedCountryCode(sourceID: String, languageCode: String) -> String {
        ...
        if source.contains("russian") && !source.contains("byelorussian") && !source.contains("belarusian") { return "RU" }
        if source.contains("byelorussian") || source.contains("belarusian") { return "BY" }
...
private enum FlagImageFactory {
    static func flag(for countryCode: String, size: NSSize = NSSize(width: 22, height: 14)) -> NSImage {
        ...
        case "BY": return belarus(size: size)
    ...
    private static func belarus(size: NSSize) -> NSImage {
        let image = NSImage(size: size)
        image.lockFocus()

        NSColor(calibratedRed: 0.76, green: 0.06, blue: 0.10, alpha: 1).setFill()
        NSBezierPath(rect: NSRect(x: 0, y: size.height * 0.33, width: size.width, height: size.height * 0.67)).fill()

        NSColor(calibratedRed: 0.02, green: 0.53, blue: 0.22, alpha: 1).setFill()
        NSBezierPath(rect: NSRect(x: 0, y: 0, width: size.width, height: size.height * 0.33)).fill()

        let ornamentWidth = max(2.0, size.width * 0.12)
        let ornamentRect = NSRect(x: 0, y: 0, width: ornamentWidth, height: size.height)

        NSColor.white.setFill()
        NSBezierPath(rect: ornamentRect).fill()

        // Simplified left ornament: red diamonds over white strip.
        NSColor(calibratedRed: 0.76, green: 0.06, blue: 0.10, alpha: 1).setFill()
        let step = max(2.0, size.height / 6.0)
        var y: CGFloat = -step
        while y < size.height + step {
            let centerX = ornamentWidth * 0.5
            let diamondW = ornamentWidth * 0.56
            let diamondH = step * 0.60
            let path = NSBezierPath()
            path.move(to: NSPoint(x: centerX, y: y + diamondH))
            path.line(to: NSPoint(x: centerX + diamondW * 0.5, y: y + diamondH * 0.5))
            path.line(to: NSPoint(x: centerX, y: y))
            path.line(to: NSPoint(x: centerX - diamondW * 0.5, y: y + diamondH * 0.5))
            path.close()
            path.fill()

            let sideW = ornamentWidth * 0.14
            NSBezierPath(rect: NSRect(x: ornamentWidth * 0.12, y: y + diamondH * 0.40, width: sideW, height: diamondH * 0.20)).fill()
            NSBezierPath(rect: NSRect(x: ornamentWidth * 0.74, y: y + diamondH * 0.40, width: sideW, height: diamondH * 0.20)).fill()
