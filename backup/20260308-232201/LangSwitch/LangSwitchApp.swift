...
private final class FlagStore {
    ...
    private func mappedCountryCode(sourceID: String, languageCode: String) -> String {
        ...
        if source.contains("australian") { return "AU" }
        if source.contains("british") || source.contains("uk") || source.contains("u.k.") { return "GB" }
        if source.contains("newzealand") || source.contains("new_zealand") { return "NZ" }
...
private enum FlagImageFactory {
    ...
    private static func heart(size: NSSize) -> NSImage {
        let image = NSImage(size: size)
        image.lockFocus()
        NSColor.white.setFill()
        NSBezierPath(rect: NSRect(origin: .zero, size: size)).fill()
